...
| S-08 | High (ops) | Durable jobs                 | Residual / OWNER  | STAGING/PRODUCTION refuse non-durable queues (D-022). See decision packet.                                                                                                                                |
...
| S-10 | Medium     | Deletion / retention         | Partial           | Synthetic deletion drill records a request, soft-deletes the operational row, revokes sessions, and retains events. D-007 durations and staging rehearsal remain. No HIPAA claim. No provider-side erase. |
...
- Synthetic deletion drill (`npm run privacy:deletion-drill`) — request + soft-delete only; events retained
