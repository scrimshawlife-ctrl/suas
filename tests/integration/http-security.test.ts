...
describe('browser response hardening', () => {
  it('applies fail-closed security and cache headers to the web and API surfaces', async () => {
    ...
    expect(web.headers['content-security-policy']).toContain("style-src 'sha256-");
    expect(web.headers['content-security-policy']).toContain("font-src 'none'");
    expect(web.headers['content-security-policy']).not.toContain("'unsafe-inline'");
    expect(web.body).not.toContain('fonts.googleapis.com');
    expect(web.body).not.toContain('fonts.gstatic.com');
